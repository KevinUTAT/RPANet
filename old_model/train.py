...
parser.add_argument("--image_folder", type=str, default="data/sparrow_nsparrow/images", help="path to dataset")
...
for epoch in range(opt.epochs):
    for batch_i, (_, imgs, targets) in enumerate(dataloader):
        ...
        # load_data = time.perf_counter()
        # print(f"load data took: {load_data - batch_start:0.4f} second")

        optimizer.zero_grad()
        # opt_time = time.perf_counter()
        # print(f"optimization took: {opt_time - load_data:0.4f} second")

        loss = model(imgs, targets)
        # loss_time = time.perf_counter()
        # print(f"get loss took: {loss_time - opt_time:0.4f} second")

        loss.backward()
        # prop_time = time.perf_counter()
        # print(f"back prop took: {prop_time - loss_time:0.4f} second")

        optimizer.step()
        # sopt_time = time.perf_counter()
        # print(f"step opt took: {sopt_time - prop_time:0.4f} second")
...
model.save_weights("final.weights")
